...
namespace OpenMafia
{
    public class ModelGenerator : BaseGenerator
    {
                ...
                foreach (var mafiaMesh in model.meshes)
                {
                    ...
                    switch (mafiaMesh.visualMeshType)
                    {
                        case MafiaFormats.VisualMeshType.VISUALMESHTYPE_STANDARD:
                        {
                            ...
                            {
                                meshFilter.mesh = GenerateMesh(mafiaMesh, mafiaMesh.standard.lods[0], model, out materials);
                        ...
                        {
                            meshFilter.mesh = GenerateMesh(mafiaMesh, mafiaMesh.singleMorph.singleMesh.standard.lods[0], model, out materials);
                            meshRenderer.materials = materials;
                        }
                        break;
        ...
        Mesh GenerateMesh(MafiaFormats.Mesh mafiaMesh, MafiaFormats.LOD firstMafiaLOD, MafiaFormats.Model model, out Material[] materials)
        {
            var mesh = new Mesh();
